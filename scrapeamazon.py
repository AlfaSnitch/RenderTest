...
def create_webdriver():
    ...
    # Specify a timeout for the HTTPConnectionPool
    pool_timeout = Timeout(connect=10, read=10)

    driver = Edge(options=options, timeout=pool_timeout)
    return driver
...
def update_graph1(btnValue, search_term):
    ...
    if search_term:
        ...
        df['brand'] = df['description'].str.split().str[0].str.lower()
...
def update_graph2(value):
    ...
    if value:
        ...
        df_brand_wise['description_1'] = df_brand_wise['description'].str.split().str[:6].apply(lambda x: ' '.join(x))
